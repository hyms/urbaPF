...
import { UserRole } from '../utils/appEnums.ts'
...
export const useAuthStore = defineStore('auth', {
  ...
  getters: {
    ...
    userRole: (state) => state.user?.role ?? UserRole.Restricted,
    ...
    isRestricted: (state) => state.user?.role === UserRole.Restricted,
    isValidated: (state) => state.user?.isValidated ?? false
  },

  actions: {
    async check(): Promise<boolean> {
      this.loadUserFromStorage() // Ensure user data is loaded on initial check
